...
export const buyCoupon = (rut, patente, cupon) => {
  return async (dispatch) => {
    ...
    const resp = await fetchSinToken(
      "cupones/obtener/" + rutNew + "/" + cupon + "/" + patente,
      data,
      "GET"
    );
    ...
    if (resp.status === 200) {
      ...
      if (resp.status === 406) {
        Swal.fire(
          "Error",
          "Cupón agotado o caducado " +
            "</BR> Ya superaste el máximo de cupones a solicitar para este servicio.",
          "error"
        );
      } else {
...
export const suscripcion = (nombre, email) => {
  return async (dispatch) => {
    ...
    if (resp.status === 200) {
      ...
      Swal.fire(
        "Error",
        "Ocurrió un error al registrar tu suscripción, Intenta de nuevo más tarde.",
        "error"
      );
    }
  };
    ...
    await fetch(`${baseUrl}/app/accidente`, requestOptions)
      ...
      .then((result) => {
        if (result.message === "OK" || result.includes("OK")) {
          ...
          Swal.fire(
            "Error",
            "Ocurrió un error al enviar tus datos, Intenta de nuevo más tarde.",
            "error"
          );
        }
      })
